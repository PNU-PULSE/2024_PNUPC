...
$N$개의 정점과 $M$개의 간선으로 이루어진 무향 단순 그래프 $G$가 존재한다. 그래프의 정점에는 가중치가 존재하며, 경로의 순서대로 정점의 가중치를 배치한 수열이 증가하는 수열인 경로를 VIP라고 하자. 이 그래프에서 $Q$개의 질의가 주어진다. 질의는 다음과 같다.

$p\ q$: 그래프 $G$에서 정점 $p$와 정점 $q$를 연결하는 간선이 존재할 경우, 그 간선을 제거했을 때 그래프의 VIP의 개수를 $10^9 + 7$로 나눈 나머지를 출력한다. 만약 간선이 존재하지 않을 경우, $p$와 $q$를 잇는 간선을 추가했을 때 그래프의 VIP의 개수를 $10^9 + 7$로 나눈 나머지를 출력한다.
